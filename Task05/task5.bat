...
echo --------------------------------------------------
Z:\sqlite3.exe movies_rating.db -box -echo "SELECT m.title, m.year, AVG(r.rating) as avg_rating, RANK() OVER (ORDER BY AVG(r.rating) DESC) as rank_by_avg_rating FROM movies m JOIN ratings r ON m.id = r.movie_id GROUP BY m.id, m.title, m.year ORDER BY rank_by_avg_rating LIMIT 10;"
echo " "
...
echo --------------------------------------------------
Z:\sqlite3.exe movies_rating.db -box -echo "SELECT user_id, COUNT(rating) as rating_count, ROUND(AVG(rating), 2) as avg_rating, datetime(MIN(timestamp), 'unixepoch') as first_rating_date, datetime(MAX(timestamp), 'unixepoch') as last_rating_date FROM ratings GROUP BY user_id ORDER BY rating_count DESC LIMIT 10;"
